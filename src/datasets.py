...
class IVM(BaseDataset):
    def __init__(self, cfg, args, device='cuda:0'
                 ):
        super(IVM, self).__init__(cfg, args, device)

        stride = cfg['stride']
        
        # load tum
        self.color_paths, self.right_color_paths, self.depth_paths = self.loadivm(
            self.input_folder, frame_rate=-1)
    ...
    def loadivm(self, datapath, frame_rate=-1):
        """ read video data in tum-rgbd format """
        # download from: https://github.com/princeton-vl/DROID-SLAM/tree/main/data/euroc_groundtruth
        scene_name = datapath.split('/')[-1]

        image_list = sorted(glob.glob(os.path.join(datapath, 'left/*.JPG')))
        right_image_list  = sorted(glob.glob(os.path.join(datapath, 'right/*.JPG')))
        depth_list = sorted(glob.glob(os.path.join(datapath, 'depth', '*.npy')))

        return image_list, right_image_list, depth_list
